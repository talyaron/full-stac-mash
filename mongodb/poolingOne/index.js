...
app.get('/api/get-all-eyeshadow', (req, res) => {
    db.collection('products').find({ type: 'eyeshadow' }, (err, docs) => {
        const results = []
        docs.each(function (err, doc) {
            if (err) console.log('Error', err);
...
app.get('/api/get-all-foundation', (req, res) => {
    db.collection('products').find({ type: 'foundation' }, (err, docs) => {
        const results = []
        docs.each(function (err, doc) {
            if (err) console.log('Error', err);
...
app.put('/api/update-price', (req, res) => {
    ...
    const newValues = { $set: { price: newPrice } };
...
app.post('/api/add-product',(req, res)=>{
    ...
    db.collection("products").insertOne(newProd , function (err, res) {
        if (err) console.log(err);
